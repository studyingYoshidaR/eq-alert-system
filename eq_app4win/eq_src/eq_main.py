...
class EqSystem:
    def __init__(self, visualizers=None, saves_history=False):
        # ロガーセットアップ
        self.logger = EqLogger.setup_logger("MainSystem", log_dir=config.LOG_DIR)
    ...
    def on_earthquake_data(self, data: JmaEqData):
        ...
        with self.lock:
            ...
            with self.audio_lock:
                ...
                threading.Thread(target=self._play_audio_thread, args=(audio_list, priority, getattr(config, "MASTER_VOLUME", 90), data.is_simulated), daemon=True).start()
    ...
    def trigger_observed_arrival(self, event_id: str):
        ...
        cd_volume = min(100, getattr(config, "MASTER_VOLUME", 90) + getattr(config, "COUNTDOWN_VOLUME_BOOST", 5))
        with self.audio_lock:
            if self.is_playing:
                if priority > self.current_playing_priority:
                    self._stop_current_audio()
