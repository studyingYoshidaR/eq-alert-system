...
class NiedMonitor:
    ...
    def __init__(self, settings: dict, callback: Callable[[JmaEqData], None], visualizers=None, saves_history:bool=False, eq_system=None, is_borehole_func=None):
        self.logger = EqLogger.setup_logger("NiedMonitor", log_dir=config.LOG_DIR)
    ...
    def _draw_status_bar(self, img: Image.Image, timestamp: str, max_intensity: float):
        ...
        try:
            font = ImageFont.truetype(
                ...
                ] if os.path.exists(p)), None) or "", 12
            )
        except:
        ...
        def worker():
            try:
                ...
                if generate_mp4:
                    ...
                    import sys
                    ffmpeg_cmd = "ffmpeg"
                    # PyInstaller同梱 (_internal/ffmpeg/ffmpeg.exe) を最優先
                    _bundled_ffmpeg = os.path.join(getattr(sys, "_MEIPASS", ""), "ffmpeg", "ffmpeg.exe")
                    if getattr(sys, "frozen", False) and os.path.isfile(_bundled_ffmpeg):
                        ffmpeg_cmd = _bundled_ffmpeg
                    elif not shutil.which(ffmpeg_cmd):
