...
class UnifiedEqApp:
    def __init__(self, root, splash=None):
        ...
        self.log_dir = os.path.join(PROJECT_ROOT, "eq_log")
        ...
        # --- データソースパス設定 ---
        self.eew_log_dir = os.path.join(PROJECT_ROOT, "eq_log", "eq_eew_jma")
        self.nied_image_log_dir = os.path.join(PROJECT_ROOT, "eq_log", "monitor_images", "eq_log")
        # フォールバック: 既存の eqlog_* ディレクトリも対応
        self.legacy_log_dir = os.path.join(PROJECT_ROOT, "eq_log")
    ...
    def _run_git_pull(self):
        """終了/再起動前に git pull を実行する"""
        import subprocess
        try:
            print("Running git pull...")
            # PROJECT_ROOT をカレントディレクトリとして git pull を実行
            subprocess.run(["git", "pull"], cwd=PROJECT_ROOT, timeout=10)
        except Exception as e:
